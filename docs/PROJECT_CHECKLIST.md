...
- [x] **Communication JS ↔ Unity (scaffold complet)** — sous-dossier `unity-client/` avec : `RocketpiBridge.cs` (singleton MonoBehaviour), `RocketpiBridge.jslib` (bridge WebGL), DTOs `ConfigPayload`/`SessionPayload`/`MatchResultPayload`, script `Tools > RocketPi > Build WebGL`, CLAUDE.md projet. Côté Laravel : `resources/js/lib/rocketpi-bridge.ts` installe `window.rocketpi.*`, `<UnityCanvas/>` consomme un token Sanctum éphémère (TTL 1h, ability `unity:*`) injecté par `PlayController`.
- [x] **Client Unity Training (scaffold complet)** — Packages/manifest (URP, Input System, Newtonsoft.Json, TestFramework, TextMeshPro), 5 asmdefs (Bridge, RestClient, Gameplay, UI, Editor), REST client `RocketpiApiClient` + DTOs, gameplay (PlayerController FPS, HealthSystem, DamageHitbox, 2 types armes, 3 abilities), 8 OperatorData ScriptableObjects alignés sur `OperatorSeeder` Laravel, `TrainingMatchManager` + `PracticeTarget` + spawner, UI (HUD, MainMenu, MatchSummary), WebGL template, scaffolders Editor (`FirstRunSetup`, `RosterScaffolder`, `SceneScaffolder`, `BuildWebGL`), 2 fichiers de tests NUnit EditMode. Reste à faire dans l'éditeur Unity humain : brancher refs UI dans l'Inspector + créer 2 prefabs (cible, arme). Voir [unity-client/README.md](../unity-client/README.md) section "Setup".
